...
#### addWorker
`execute`方法需要创建新线程执行任务时会调用`addWorker`方法，会将任务封装为一个`Worker`对象，加入`HashSet`中。`Worker`是继承了`Runable`接口的内部类，对工作线程进行了封装。
```java
...
关闭（销毁）线程池，`shutdown()`会将线程池状态设置为`SHUTDOWN`，拒绝新任务加入，终端空闲线程，但已在队列里的任务会继续处理。`shutdownNow()`方法则会强行关闭线程池，尝试将线程池状态设置为`STOP`，会中断所有线程。
