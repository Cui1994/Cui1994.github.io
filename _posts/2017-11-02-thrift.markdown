...
category: "烂笔头"
title:  "thriftpy的应用（一）"
tags: [Python, thrift, SOA]
---

# Thrift是什么

thrift是一款由Apache开发的开源RPC框架，适用于跨语言服务间的调用通信。

# 工作机制

thrift框架通过一层中间语言（IDL，接口定义语言）来定义RPC的数据类型和接口，并可以生成不同语言的代码来实现服务调用。
...
是一款由饿了么公司开发的开源框架，封装了thrift协议层的一些内容，可以直接读取thrift描述文件并生成python对象，可以获得其中的数据结构和服务接口。
...
service New {
    string ping()
        throws (1: OperationException operation_exception),
}
```
...
new_thrift = thriftpy.load("new.thrift", module_name="new_thrift")

from thriftpy.rpc import make_server

OperationException = new_thrift.OperationException
...
server = make_server(new_thrift.New, Dispatcher(), '127.0.0.1', 5000)
server.serve()
```
...
new_thrift = thriftpy.load("new.thrift", "new_thrift")

from thriftpy.rpc import make_client

OperationException = new_thrift.OperationException

client = make_client(new_thrift.New, "127.0.0.1", 5000)
